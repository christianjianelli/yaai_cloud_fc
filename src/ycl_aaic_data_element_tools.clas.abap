...
CLASS ycl_aaic_data_element_tools IMPLEMENTATION.
  ...
  METHOD create.
    ...
    IF i_domain_name IS INITIAL AND i_data_type IS INITIAL.
      r_response = |For the Data Element type you must specify either an ABAP Domain or an ABAP built-in type. { NEW ycl_aaic_ddic_tools_util( )->get_built_in_types_response( ) }|.
      RETURN.
    ENDIF.
  ...
  METHOD update.
    ...
    DATA(lo_specification) = lo_patch_operation->for-dtel->add_object( l_data_element_name
    ...
    IF i_label_short IS NOT INITIAL.
      lo_specification->for-update->field_label-short->set_text( CONV #( i_label_short ) ).
    ENDIF.

    IF i_label_medium IS NOT INITIAL.
      lo_specification->for-update->field_label-medium->set_text( CONV #( i_label_medium ) ).
    ENDIF.

    IF i_label_long IS NOT INITIAL.
      lo_specification->for-update->field_label-long->set_text( CONV #( i_label_long ) ).
    ENDIF.

    IF i_label_heading IS NOT INITIAL.
      lo_specification->for-update->field_label-heading->set_text( CONV #( i_label_heading ) ).
    ENDIF.

    IF l_domain_name IS NOT INITIAL.

      lo_specification->for-update->set_data_type( xco_cp_abap_dictionary=>domain( l_domain_name ) ).

    ELSEIF lo_format IS BOUND.

      lo_specification->for-update->set_data_type( lo_format ).
  ...
  METHOD delete.
    ...
    DATA(lo_delete_operation) = xco_cp_generation=>environment->dev_system( l_transport_request )->for-dtel->create_delete_operation( ).

    lo_delete_operation->add_object( l_data_element_name ).
...
*          i_domain_name        = 'ZDO_CJS_EMAIL'
          ...
          i_length             = '200'
*          i_decimals           = '3'
*          i_label_short        = 'EmailFrom'
*          i_label_medium       = 'Email From A'
*          i_label_long         = 'Email From B'
*          i_label_heading      = 'Email From C'
          i_transport_request  = 'TRLK900008'
      ).
