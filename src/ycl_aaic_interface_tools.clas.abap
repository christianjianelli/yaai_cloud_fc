  ...
  PRIVATE SECTION.
    ...
    METHODS _get_methods
      IMPORTING
        io_interface             TYPE REF TO if_xco_ao_interface
      EXPORTING
        e_t_class_methods_public TYPE sxco_t_intf_c_methods
        e_t_methods_public       TYPE sxco_t_intf_c_methods.
...
CLASS ycl_aaic_interface_tools IMPLEMENTATION.
  ...
  METHOD create.
    ...
    DATA(lo_specification) = lo_put_operation->for-intf->add_object( l_interface_name
    ...
    TRY.
        ...
        r_response = |Interface `{ l_interface_name }` created successfully!|.

      CATCH cx_xco_gen_put_exception INTO DATA(lo_cx_xco_gen_put_exception).

        r_response = |Error: the interface `{ l_interface_name }` was not created.|.

        me->_add_findings_to_response(
          EXPORTING
            i_o_findings = lo_cx_xco_gen_put_exception->findings->for->intf
          CHANGING
  ...
  METHOD add_method.
    ...
    DATA(l_method_name) = CONV sxco_ao_component_name( condense( to_upper( i_method_name ) ) ).

    IF xco_cp_abap=>interface( l_interface_name )->exists( ) = abap_false.
    ...
    DATA(lo_patch_operation) = xco_cp_generation=>environment->dev_system( l_transport_request )->for-intf->create_patch_operation( ).

    DATA(lo_patch_operation_object) = lo_patch_operation->add_object( l_interface_name ).

    IF i_static = abap_false.

      DATA(lo_method_definition) = lo_patch_operation_object->for-insert->add_method( l_method_name
        )->set_short_description( i_description ).

    ELSE.

      lo_method_definition = lo_patch_operation_object->for-insert->add_class_method( l_method_name
        )->set_short_description( i_description ).
  ...
  METHOD add_method_parameters.
    ...
    DATA(l_static) = lo_interface->component->method( l_method_name )->exists( ).

    IF l_static = abap_false.

      DATA(lo_method_definition) = lo_patch_operation_object->for-update->add_method( l_method_name ).

    ELSE.

      lo_method_definition = lo_patch_operation_object->for-update->add_class_method( l_method_name ).
  ...
  METHOD delete_method_parameters.
    ...
    DATA(l_static) = lo_interface->component->class_method( l_method_name )->exists( ).
  ...
  METHOD delete_method.
    ...
    IF l_static = abap_false.
      ...
      lo_patch_operation_object->for-delete->add_method( l_method_name ).

    ELSE.

      lo_patch_operation_object->for-delete->add_class_method( l_method_name ).
  ...
  METHOD add_attribute.
    ...
    IF i_static = abap_false.
      ...
      DATA(lo_data) = lo_patch_operation_object->for-insert->add_data( l_attribute_name
    ...
    ELSE.
      ...
      DATA(lo_class_data) = lo_patch_operation_object->for-insert->add_class_data( l_attribute_name
        )->set_type( xco_cp_abap=>type-source->for( i_attribute_type )
        )->set_read_only( i_read_only ).
  ...
  METHOD delete_attribute.
    ...
    DATA(l_static) = lo_interface->component->class_data( l_attribute_name )->exists( ).

    IF l_static = abap_false.

      lo_patch_operation_object->for-delete->add_data( l_attribute_name ).

    ELSE.

      lo_patch_operation_object->for-delete->add_class_data( l_attribute_name ).
  ...
  METHOD get_interface_definition.
    ...
    TRY.
        ...
        DATA(lt_public_types) = lo_interface->components->type->all->get( ).

        DATA(lt_public_constants) = lo_interface->components->constant->all->get( ).
      ...
      CATCH cx_xco_runtime_exception ##NO_HANDLER.
        ...
        r_response = |Error while reading the definition of the interface `{ l_interface_name }`.|.
...
*      r_response = |{ r_response }{ cl_abap_char_utilities=>newline }|.
  ...
  METHOD get_method_definition.
    ...
    IF l_static = abap_false.
      ...
      IF lo_interface->component->method( l_method_name )->exists( ) = abap_false.
        r_response = |The method `{ l_method_name }` does not exist in interface `{ l_interface_name }`.|.
        RETURN.
      ENDIF.
    ...
    TRY.
        ...
        IF l_static = abap_true.
          ...
          DATA(lo_method) = lo_interface->component->class_method( l_method_name ).

        ELSE.

          lo_method = lo_interface->component->method( l_method_name ).

        ENDIF.

      CATCH cx_xco_runtime_exception ##NO_HANDLER.

        r_response = |Error while reading the definition of the method `{ l_method_name }` of interface `{ l_interface_name }`.|.
  ...
  METHOD _get_package.
    ...
    DATA(lo_interface) = xco_cp_abap=>interface( i_interface_name ).

    IF lo_interface->exists( ) = abap_false.
      RETURN.
    ENDIF.

    DATA(lo_package) = lo_interface->if_xco_ar_object~get_package( ).
  ...
  METHOD _get_transport_request.
    ...
    DATA(lo_lock) = lo_interface->if_xco_cts_changeable~get_object( )->get_lock( ).
  ...
  METHOD _get_attributes.
    ...
    e_t_class_datas_public = io_interface->components->class_data->all->get( ).

    e_t_datas_public = io_interface->components->data->all->get( ).

  ENDMETHOD.

  METHOD _get_methods.

    e_t_class_methods_public = io_interface->components->class_method->all->get( ).

    e_t_methods_public = io_interface->components->method->all->get( ).
